...
JOBLIB_BACKEND = "loky" # Set joblib.Parallel backend to "loky". loky is the default backend, specifically designed to handle Python's GIL (Global Interpreter Lock), suitable for multi-process computation tasks
...
def parse_inst_info(df):
    ...
    inst_type_str = df["inst_type"] # Instrument type
    inst_str = df["inst"] # Instrument name
    frag_mode_str = df["frag_mode"] # Fragmentation mode
    col_energy_str = df["col_energy"] # Collision energy
    # instrument type 
    if inst_type_str == "EI":
        assert inst_str == "EI"
        ...
        return "EI", "EI"
    if none_or_nan(inst_type_str): # If inst_type is empty (None or NaN), try to infer from inst (instrument name)
        # resort to instrument # Infer type based on instrument name
        inst_map = {
        ...
        if none_or_nan(inst_str):
            inst_type = np.nan # If instrument name is empty, return NaN
        elif inst_str in inst_map:
            inst_type = inst_map[inst_str]
        else:
            inst_type = "Other"
    else:
        inst_type_map = { # Standardize directly based on instrument type
            "QTOF": "QTOF",
            "FT": "FT",
            "Q-TOF": "QTOF",
            "HCD": "FT",
        ...
        if inst_type_str in inst_type_map:
            ...
            inst_type = "Other"
    # fragmentation mode  Standardize fragmentation mode, infer fragmentation mode based on input data
...
def parse_peaks_str(peaks_str):
    """The function of this code is to parse spectral peak data from string to structured data (usually in list form)"""
    # peaks still represented as string
    if none_or_nan(peaks_str):
        return np.nan
    # lines = peaks_str.split("\n") # Split peaks_str by newline character \n into multiple line strings, each line usually represents a peak data point and peak structure name
    ...
    for line in lines:
        ...
        mz = line[0]
        ints = line[1]
        peaks.append((mz, ints)) # Add the parsed mass-to-charge ratio and intensity as tuple (mz, ints) to the list peaks
    return peaks
...
def get_res(peaks):
    """The function of this code is to calculate the peak with the highest resolution in mass spectrum peak data. Resolution is defined as the number of significant decimal places of mz (mass-to-charge ratio)"""
    # assumes no nan
    ress = [] # Create an empty list ress to store the resolution of each mz
    for mz, ints in peaks:
        dec_idx = mz.find(".")
        if dec_idx == -1:
            res = 0
        else:
            res = len(mz) - (dec_idx + 1) # Number of decimal places equals the total length of mz minus the position after the decimal point
        ress.append(res)
