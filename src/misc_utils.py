...
def flatten_lol(list_of_list):
    # flatten_lol is a simple function used to flatten a nested list (list of list) into a single-layer flat list
    flat_list = []
    for ll in list_of_list:
        flat_list.extend(ll)
    return flat_list

# Uses the contextlib.contextmanager decorator. This context manager is used to temporarily set the seed of NumPy's random number generator, generally used with the with statement. After the yield statement, control is passed to the code block within the with statement
# Once the code block within the with statement completes execution, control returns to the context manager. If an exception occurs in the code block, the exception will be thrown and caught externally, but before that, the finally block ensures the restoration of the random state
...
def get_scaler(amp):
    # Whether to enable mixed precision training (AMP), returns an appropriate gradient scaler object for optimizing gradient computation in backpropagation
    if amp:
        scaler = th.cuda.amp.GradScaler() # Enable AMP, return PyTorch's GradScaler instance
    else:
        scaler = DummyScaler() # Disable AMP, return DummyScaler (a placeholder class that performs no scaling operations)
    return scaler
